...
function renderEngageDropdown(page) {
  const active = ENGAGE_LINKS.some((link) => link.href === page)
    ? " active"
    : "";
  const submenu = ENGAGE_LINKS.map((link) => {
    const activeItem = link.href === page ? " active" : "";
    return `<a href="${link.href}" class="nav-submenu-link${activeItem}" ${attr(link.en, link.km)}>${link.en}</a>`;
  }).join("");

  return `
    <div class="nav-dropdown${active}">
      <button class="nav-link nav-dropdown-toggle${active}" type="button" aria-expanded="false" ${attr("Get Involved", "ចូលរួម")}>
        Get Involved
      </button>
...
function renderHeader() {
  ...
  const links = NAV_LINKS.map((link) => renderNavLink(link, page)).join("");
  const engageDropdown = renderEngageDropdown(page);
    ...
    <footer>
      <div class="container footer-grid">
        ...
        <div class="footer-socials">
          <h4 ${attr("Our Socials", "បណ្តាញសង្គមរបស់យើង")}>Our Socials</h4>
          <ul>
            <li><a href="https://www.instagram.com/neurocarecambodia/" target="_blank" rel="noopener" ${attr("Instagram: @neurocarecambodia", "Instagram៖ @neurocarecambodia")}>Instagram: @neurocarecambodia</a></li>
            <li><a href="https://www.facebook.com/neurocarecambodia" target="_blank" rel="noopener" ${attr("Facebook: NeuroCare Cambodia", "Facebook៖ NeuroCare Cambodia")}>Facebook: NeuroCare Cambodia</a></li>
            <li><a href="https://www.tiktok.com/@neurocarecambodia" target="_blank" rel="noopener" ${attr("TikTok: @neurocarecambodia", "TikTok៖ @neurocarecambodia")}>TikTok: @neurocarecambodia</a></li>
            <li><a href="https://www.youtube.com/@neurocarecambodia" target="_blank" rel="noopener" ${attr("YouTube: NeuroCare Cambodia", "YouTube៖ NeuroCare Cambodia")}>YouTube: NeuroCare Cambodia</a></li>
          </ul>
        </div>
      </div>
